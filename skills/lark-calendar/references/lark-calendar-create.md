...
| `--description-rich <markdown>` | 否 | 日程描述，统一使用此字段，格式为 **Markdown**。提供会议议程、活动内容、注意事项或链接等。支持加粗、斜体、下划线（`<u>...</u>`）、删除线、链接 `[文本](url)`、有序/无序列表、以及 GFM 表格（`\| 列1 \| 列2 \|` + 分隔行 `\| --- \| --- \|`）。飞书文档 URL（直接粘贴裸链接，或写成 `[文本](url)`）会自动解析为内联文档，端上展示文档标题而非裸链接。支持 `@文件路径` 或 `-`（stdin）读取。|
| `--description <text>` | 否 | 【已废弃，隐藏】纯文本描述，请改用 `--description-rich`（Markdown）|
